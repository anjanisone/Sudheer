...
def clean_all_date_fields(df):
    """
    Cleans and standardizes all date/timestamp fields to ISO 8601.
      - Trims & normalizes raw strings
      - Replaces placeholders with null
      - Parses numeric and month-name formats
      - Nullifies out-of-range years (<1900 or > current year) and future values (to current second)
      - Formats:
    ...
    # Heuristic: treat any column whose name includes 'date', 'timestamp', or 'time' as date-like
    date_cols = [c for c in df.columns if any(x in c.lower() for x in ["date", "timestamp", "time"])]

    for c in date_cols:
        # Trim & drop placeholders
        df = df.withColumn(c, F.trim(F.col(c)))
        df = df.withColumn(c, F.when(F.col(c).isin(invalid_values), None).otherwise(F.col(c)))

        # Parse formats (timestamps first, then dates); include month-name variants
        parsed = F.coalesce(
            # timestamp-like
        ...
        # Nullify out-of-range and future (compare value to 'now')
        df = df.withColumn(
            c,
            F.when(
        ...
        # Output ISO 8601 (by name heuristic)
