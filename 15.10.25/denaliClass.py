...
class DenaliDataProcessor:
    def __init__(self, glue_context):
        ...
        self.PREFIX_ADDRESS = "ush_denali_addresses"
        self.PREFIX_CONTACTS = "ush_denali_cust_contacts"
        self.contacts_schema = StructType([
                ...
                StructField("LastFourIDNo", StringType(), True)
            ])
        ...
        self.columns_to_select = {
            ...
            self.PREFIX_CONTACTS: [
                ...
                "MiddleName",
                "LastName",
                "Phone", # replaced Cell with phone as per CR-26
                "Email",
                "DOB",
                "Gender",
                "AddressID",
        ...
        # self.join_keys = ["AddressID"]
        # Email columns have case sensitivity
    ...
    def process_and_join_files(self, filepaths: List[str]) -> DataFrame:
        ...
        for base_name, files in file_groups.items():
            ...
            if df is not None:
                ...
                if self.dedupekey in df.columns:
                    ...
                    # Drop dedupekey column for all prefixes except "ush_denali_cust_contacts"
                    if prefix != "ush_denali_cust_contacts":
                        df = df.drop(self.dedupekey)
                else:
                    logger.warning(f"Warning: {self.dedupekey} column not found in DataFrame for prefix '{prefix}'. Skipping deduplication.")
        ...
        # Define join mapping
        join_mapping = {
                "ush_denali_cust_contacts": "AddressID",
                "ush_denali_addresses": "AddressID"
            }
    ...
    def join_dataframes(self, df_dict, join_mapping):
        # Start with the ush_denali_cust_contacts DataFrame as the base
        base_df_name = 'ush_denali_cust_contacts'
        ...
        # Select columns, keeping only the AddressID from the base DataFrame
        columns_to_select = []
        for column in base_df.columns:
            if column.endswith('.AddressID'):
                if column.startswith('base.'):
                    columns_to_select.append(col(column).alias('AddressID'))
            else:
